...
void UNetworkPlayerComponent::BeginPlay()
{
    ...
    if (bAutoConnectOnBeginPlay)
    {
        ...
        GetWorld()->GetTimerManager().SetTimer(
            ConnectTimerHandle,
            this,
            &UNetworkPlayerComponent::ConnectToServer,
            1.0f,
            false
        );
    }
...
void UNetworkPlayerComponent::BindNetworkEvents()
{
    ...
    // 새로운 델리게이트만 바인딩
    NetworkManager->OnPlayerUpdate.AddDynamic(this, &UNetworkPlayerComponent::OnPlayerUpdateReceived_Implementation);
    NetworkManager->OnClientIdReceived.AddDynamic(this, &UNetworkPlayerComponent::OnClientIdReceived_Implementation);
...
void UNetworkPlayerComponent::UnbindNetworkEvents()
{
    ...
    // 모든 델리게이트 바인딩 제거
    NetworkManager->OnPlayerUpdate.RemoveDynamic(this, &UNetworkPlayerComponent::OnPlayerUpdateReceived_Implementation);
    NetworkManager->OnClientIdReceived.RemoveDynamic(this, &UNetworkPlayerComponent::OnClientIdReceived_Implementation);
...
// 인터페이스 구현
void UNetworkPlayerComponent::OnNetworkConnected_Implementation()
{
    // 블루프린트에서 오버라이드 가능
    UE_LOG(LogTemp, Display, TEXT("Network connected"));
}

void UNetworkPlayerComponent::OnNetworkDisconnected_Implementation()
{
    // 블루프린트에서 오버라이드 가능
...
void UNetworkPlayerComponent::OnPlayerUpdateReceived_Implementation(int32 ClientId, const FVector& Position, const FRotator& Rotation, const FVector& Velocity, bool IsJumping)
{
    // 자신의 업데이트는 무시
...
void UNetworkPlayerComponent::OnClientIdReceived_Implementation(int32 ClientId)
{
    // 로컬 클라이언트 ID 저장
...
void UNetworkPlayerComponent::OnOtherPlayerSpawned_Implementation(AActor* OtherPlayerActor, int32 ClientId)
{
    // 블루프린트에서 오버라이드 가능
    UE_LOG(LogTemp, Display, TEXT("Other player spawned - Client ID: %d"), ClientId);
}

void UNetworkPlayerComponent::OnOtherPlayerRemoved_Implementation(int32 ClientId)
{
    // 블루프린트에서 오버라이드 가능
